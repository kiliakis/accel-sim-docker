...
RUN apt-get update -y && \
    apt-get install -y build-essential apt-utils wget vim htop git curl python \
        ...
        libglu1-mesa-dev g++ curl gdb libssl-dev libxml2-dev libboost-all-dev \
    ...
    sh cuda_11.0.1_450.36.06_linux.run --silent --toolkit --toolkitpath=/root/install/cuda --samples --samplespath=/root/ && \
